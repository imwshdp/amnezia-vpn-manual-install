...
Подробное руководство по ручной установке AmneziaWG протокола и поднятия AmneziaVPN на сервере с настройкой конфигурационных файлов на стороне сервера и клиента без каких-либо установочных скриптов, передачи данных и SSH-ключей вашего VPS клиенту Amnezia. Все компилируется и поднимается своими руками.
...
AmneziaVPN использует собственный протокол amneziawg-go, который является форком [https://github.com/WireGuard/wireguard-go](wireguard-go) с возможностью настройки параметров обфускации WireGuard-трафика в конфигурационных файлах. Так как эта реализация написана на языке программирования Go, для компиляции исходных файлов нам необходимо установить Go на наш сервер.
...
> [!IMPORTANT]
> Заметьте, что команды **PostUp** и **PostDown** устанавливают NAT-правила для интерфейса **wg0**. Имя конфигурационного файла должно совпадать с именем WireGuard-интерфейса, который вы поднимаете через `wg-quick` (например, **wg0.conf** → интерфейс **wg0**).
...
После поднятия AmneziaWG, просматриваем таблицу IP-роутов:
